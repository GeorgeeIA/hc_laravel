                ...
                <div class="container-fluid">
                    ...
                    <div class="card shadow mb-4">
                        <div class="card-header py-3">
                            <h6 style="color: #F78B4E !important" class="m-0 font-weight-bold text-primary">Identitas Dokter</h6>
                        </div>
                        <div class="card-body">
                            <form action="{{ route('dokter.update',['dokter' => $dokter->id]) }}" method="POST" style="color: black;">
                            @method('PATCH')
                            @csrf
                                <div class="form-group mb-3">
                                    <label for="id">ID Dokter</label>
                                    <input type="number" class="form-control @error('id') is-invalid @enderror"
                                    id="id" name="id" value="{{ old('id') ?? $dokter->id }}">
                                    @error('id')
                                ...
                                <div class="form-group mb-3">
                                    ...
                                    id="nama" name="nama" value="{{ old('nama') ?? $dokter->name }}">
                                    @error('nama')
                                    <div class="text-danger">{{ $message }}</div>
                                    @enderror
                                </div>
                                <div class="form-group">
                                    <div class="form-group mb-3">
                                        ...
                                        <select name="jeniskelamin" id="jeniskelamin" class="form-control">
                                            <option value=""{{ (old('jeniskelamin') ?? $dokter->jeniskelamin)==
                                            'pilih' ? 'selected': '' }} > pilih</option>
                                            <option value="Laki-laki"{{ (old('jeniskelamin') ?? $dokter->jeniskelamin)==
                                            'Laki-laki' ? 'selected': '' }} > Laki-laki</option>   
                                            <option value="Perempuan"{{ (old('jeniskelamin') ?? $dokter->jeniskelamin)==
                                ...
                                <div class="form-group mb-3">
                                    ...
                                    name="alamat">{{ old('alamat') ?? $dokter->alamat}}</textarea>
                                </div>
                                <div class="form-group mb-3">
                                    <label for="nohp">No Telepon</label>
                                    <input type="number" class="form-control @error('nohp') is-invalid @enderror"
                                    id="nohp" name="nohp" value="{{ old('nohp') ?? $dokter->nohp}}">
                                    @error('nohp')
                                    <div class="text-danger">{{ $message }}</div>
                                    @enderror
                                </div>
                                <div class="form-group mb-3">
                                    ...
                                    <select name="poli" id="poli" class="form-control">
                                        <option value=""{{ (old('poli') ?? $dokter->poli)==
                                        'pilih' ? 'selected': '' }} > pilih</option>                                        
                                        <option value="Gigi"{{ (old('poli') ?? $dokter->poli)==
                                        'Gigi' ? 'selected': '' }} > Gigi</option>                                        
                                        <option value="Umum"{{ (old('poli') ?? $dokter->poli)==
                                        'Umum' ? 'selected': '' }} > Umum</option>                                       
                                        <option value="Anak"{{ (old('poli') ?? $dokter->poli)==
                                        'Anak' ? 'selected': '' }} > Anak</option>                                       
                                        <option value="Gizi"{{ (old('poli') ?? $dokter->poli)==
                                        'Gizi' ? 'selected': '' }} > Gizi</option>
                                    </select>
                                </div>	
                                <button style="color: #ffffff; background-color: #F78B4E;" type="submit" class="btn">Simpan</button>
                                <a href="{{ route('dokter.index') }}" class="btn btn-primary">Kembali</a></br>
                              </form>
                        </div>
                    </div>
